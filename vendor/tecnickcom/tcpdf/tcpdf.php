 ...
 */
class TCPDF {
    protected $attached_image_stack = array();
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
        ...
        if (preg_match('/^http/',$file)) {
            ...
            if (preg_match('/cid\:/',$file)) {
                $file = substr($file, 4);
                if (!empty($this->attached_image_stack[$file])) {
                    $imgdata = $this->attached_image_stack[$file];
                } else {
                    return false;
                }
            } else {
...
//				$this->Error('[Image] Unable to get the size of the image: '.$file);
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'th') OR ($dom[$key]['value'] == 'td')) {
                        ...
                        if (!isset($dom[($dom[$key]['parent'])]['cols'])) {
                            ...
                            $p_needle = $dom[$key]['parent'];
                            while($p_needle) {
                                if (($dom[$p_needle]['value'] == 'tr') OR ($dom[$p_needle]['value'] == 'table')) {
                                    if (!isset($dom[$p_needle]['cols'])) {
                                        $dom[$p_needle]['cols'] = 1;
                                    }
                                    $dom[$p_needle]['cols'] += $colspan;
                                    break;
                                }

                                $p_needle = $dom[$p_needle]['parent'];
                            }
                        } else {
				...
				if (!empty($dom[$dom[$key]['parent']]['text-transform'])) {
					...
					if (!empty($this->encoding) AND function_exists('mb_convert_case')) {
	 ...
	 * @public
	 */
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='', $attached_image_data = []) {
        if (!empty($attached_image_data)) {
            $this->attached_image_stack = array_merge($this->attached_image_stack, $attached_image_data);
        }
...
//                        if (strpos($cell_content,'Hi Tony')) {
//                            echo PHP_EOL.'writeHTML->Multicell td/th: '.$cellw.' | '.$cellh.' | '.$cell_content;
//                            echo '**********'.PHP_EOL.PHP_EOL;
//                            echo '$this->lasth: '.$this->lasth.PHP_EOL;
//                            $temp_key = $key;
//                            while($temp_key) {
//                                echo $temp_key.': '.$dom[$temp_key]['value'].PHP_EOL;
////                              echo print_r($dom[$temp_key],true).PHP_EOL.'**********'.PHP_EOL.PHP_EOL;
//                                $temp_key = $dom[$temp_key]['parent'];
//                            }
//                            echo '**********'.PHP_EOL.PHP_EOL;
//                        }
...
//                        if (strpos($dom[$key]['value'],"Hi Tony") !== false){         // Debug code Allen
////                            echo print_r($this,true).PHP_EOL.'**********'.PHP_EOL;
//                            foreach ($dom as $temp_key=>$temp_dom_element) {
//                                echo $temp_key.': ';
//                                if (isset($dom[$temp_key]['value'])) {
//                                    echo $dom[$temp_key]['value'];
//                                }
//                                if (isset($dom[$temp_key]['parent'])) {
//                                    echo ' < '.$dom[$temp_key]['parent'];
//                                }
//                                if (isset($dom[$temp_key]['attribute']) AND isset($dom[$temp_key]['attribute']['class'])) {
//                                    echo ' -- ['.$dom[$temp_key]['attribute']['class'].']';
//                                }
//                                echo PHP_EOL;
//                            }
//                            echo '**********'.PHP_EOL.PHP_EOL;
