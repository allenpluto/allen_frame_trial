...
//$entity_eml_obj->get(['where'=>['id = 3422']]);
//$entity_eml_obj->get(['where'=>['id = 6485']]);
//$entity_eml_obj->get(['where'=>['id = 189184']]);     // Test warning for Undefined index: startcolumn
$entity_eml_obj->get(['where'=>['id = 189219']]);     // Test warning for &quote;&quote;
//18139, 19408     // with image cid
//$entity_eml_obj->get(['where'=>['id = 19408']]);     // with image cid
...
foreach ($entity_eml_obj->row as $eml_row) {
    set_time_limit(300);
echo $eml_row['id'].': '.$eml_row['subject'].PHP_EOL;
    $entity_eml_obj2 = new entity_eml();
//    $entity_eml_obj2->set_row(['id'=>$eml_row['id'],'description'=>-55]);
...
//    }
    ...
    if (file_exists($target_folder.$target_file_name.'.pdf')) {
        ...
        if (file_exists($target_folder.$target_file_name.'_HTML.pdf')) {
            unlink($target_folder.$target_file_name.'_HTML.pdf');
        }
...
//        $eml_row['content_html'] = preg_replace('/\"(.*?)\&quot\;(.*?)\"/','"$1$2"',$eml_row['content_html']);
//        echo htmlentities($eml_row['content_html']).PHP_EOL;
...
//        $entity_eml_obj2->set_row(['id'=>$eml_row['id'],'target_file'=>$target_file_name.'.pdf']);
...
//                    }
                ...
                if (!empty($eml_row['content_html']) AND !empty($attached_images)) {
                    $pdf = new TCPDF('P', 'mm', 'A4', 'UTF-8', false);
                    $pdf->SetCreator('MGS');
                    $pdf->SetAuthor('MGS');
                    $pdf->SetTitle($eml_row['subject']);
...
//        $pdf->MultiCell(190, 5, $eml_row['content_html'], 0, 'L', 0, 0, '', '', true, 0, true, true, 0, 'T', false);
                    ...
                    $pdf->Output($target_folder.$target_file_name.'_INLINE.pdf', 'F');
                }
            }
        }

//        $entity_eml_obj2->set_row(['id'=>$eml_row['id'],'target_file'=>$target_file_name.'.pdf','attachments'=>$eml_row['attachments'],'description'=>-56]);
