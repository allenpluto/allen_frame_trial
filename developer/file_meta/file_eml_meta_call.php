...
<script>
    ...
    $(document).ready(function(){
        ...
        if (isset($_GET['step']))
        {
            $option['step'] = $_GET['step'];
        }
        ?>
        var query_limit = 250000;
        var limit = <?=$option['limit']?>;
        var step = <?=empty($option['step'])?'null':'"'.$option['step'].'"'?>;
        ...
        function call_handler(){
            ...
            if (step) {
                post_value['step'] = step;
            }
            // console.log('post_value',post_value);
            $.ajax({
                'type': 'POST',
                'url': '/developer/file_meta/file_eml_meta_handler.php',
                'data': post_value,
                'timeout': 300000,
                ...
                'complete': function()
                {
                    ...
                    if (counter >= max_count)
                    {
                        ...
                        content_container.children('div:gt(10)').remove();
                        call_handler();
                    }
                }
            });
