<div id="article_category_block_container_[[*id]]" class="block_container article_category_block_container[[*extra_classes]]">
    <div class="block_thumb_container">
        <div class="block_thumb_image_container"></div>[[image:object=`view_article_category_image`:field=`{"article_category_id":"[[*id]]"}`:page_size=`1`:page_number=`0`]]
        <div class="block_thumb_content_container">
            <div class="block_thumb_content"><h3 class="block_thumb_title">[[*name]]</h3></div>
        </div>
    </div>
    <div class="clear"></div>
    <div class="block_cover_over_link_container">
        <a href="[[*base]]listing/find/[[*friendly_uri]]" title="[[*name]]" class="block_thumb_cover_over_link"></a>
    </div>
</div>